...
class HardNegativeMiner:
    ...
    def sample_hard_negatives(self, num_hard_negatives, criteria):
        ...
        # 1) Build the table (as described in the docstring)
        ...
        if criteria not in self.table.columns:
            raise KeyError(f"{criteria} not found in columns: {list(self.table.columns)}")


        # 2) Sort by criteria (hardest first)
        ...
        # 3) Take top-N (head handles "fewer than requested" automatically)
        sampled = sorted_table.head(int(num_hard_negatives)).copy()
